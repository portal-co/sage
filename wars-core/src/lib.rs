...
pub fn render_sig(m: &Module, n: SignatureData, opts: Opts) -> proc_macro2::TokenStream {
    let mut p = vec![];
    let mut r = vec![];
...
pub fn new_state_enum(f: &FunctionBody) -> proc_macro2::TokenStream {
    let mut c = vec![];
...
pub fn lower_module(
    ...
    m.expand_all_funcs()?;
    waffle::passes::tcore::trampoline_module(&mut m, true)?;
    waffle::passes::tcore::tcore(&mut m, true)?;
    ...
    for f in m.funcs.iter() {
        ...
        match m.funcs[f].clone() {
            waffle::FuncDecl::Import(s, _) => {
                ...
                let i = i.unwrap();
                let n = render_sig(&m, m.signatures[s].clone(), opts);
            ...
            waffle::FuncDecl::Body(s, _, mut b) => {
                ...
                for k in b.blocks.entries().map(|a| a.0).collect::<Vec<_>>() {
                    ...
                    bs.push(lower_body(&m, f, &b, k, wrap.clone(), opts)?);
                    // });
                }
...
pub fn lower_body(
        ...
        match b.values[*v].clone() {
            ...
            waffle::ValueDef::Operator(o, r, _) => {
                ...
                match o {
                    ...
                    waffle::Operator::F32Neg => unop!(bl,vn,ra,args,(0-#ra)),
                    ...
                    waffle::Operator::F64Neg => unop!(bl,vn,ra,args,(0-#ra)),
                    ...
                    waffle::Operator::MemorySize { mem } => {
                        ...
                        bl = quote! {
                            #bl;
                            let #vn = (self.#m().len() / 65536) as u32;
                        }
                    }
                    waffle::Operator::MemoryGrow { mem } => {
                        ...
                        bl = quote! {
                            #bl;
                            let #vn = self.#m().len() / 65536;
                            let l = self.#m().len();
                            self.#m().resize(l + (#a as usize) * 65536,0);
                            let #vn = #vn as u32
                        }
                    }
                    waffle::Operator::MemoryCopy { dst_mem, src_mem } => {
                        ...
                        ternop!(bl,vn,ra,rb,rc,args,({
                            let v = self.#src_mem()[(#ra as usize)..][..(#rc as usize)].to_vec();
                            self.#dst_mem()[(#rb as usize)..][..(#rc as usize)].copy_from_slice(&v);
                            0u32
                        }))
                    }
                    waffle::Operator::MemoryFill { mem } => {
                        ...
                        ternop!(bl,vn,ra,rb,rc,args,({
                            self.#mem()[(#ra as usize)..][..(#rc as usize)].fill((#rb & 0xff) as u8);
                            0u32
                        }))
                    }
                }
    ...
    fn block_target(
        ...
        quote! {
            {
            state = State::#f(#(#values),*);
            continue;
            }
        }
    }
    let t = match &b.blocks[a].terminator {
        waffle::Terminator::Br { target } => block_target(b, f, m, target),
        waffle::Terminator::CondBr {
            ...
            let if_true = block_target(b, f, m, if_true);
            let if_false = block_target(b, f, m, if_false);
            quote! {
                if #cond != 0{
                    #if_true
                }else{
        ...
        waffle::Terminator::Select {
            ...
            let default = block_target(b, f, m, default);
            let mut t2 = BTreeMap::new();
            let mut i = 0u32;
            for t in targets.iter() {
                t2.insert(i, block_target(b, f, m, t));
                i += 1;
            }
            let mut b = quote! {};
            for (k, v) in t2.iter() {
